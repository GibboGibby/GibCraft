...
{
public:
	GenThread(std::shared_ptr<std::map<std::pair<int, int>, std::shared_ptr<Chunk>>> worldChunks, std::shared_ptr<FPSCamera> camera, int world_seed, int render_dist = 6, int build_dist = 10);
	...
	ChunkDataTypePtr GetChunkDataForMeshing(std::shared_ptr<std::map<std::pair<int, int>, std::shared_ptr<Chunk>>> worldChunks, int cx, int cz);
